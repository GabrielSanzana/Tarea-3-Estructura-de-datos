...
void eraseMap(HashMap *map, char *key) {
    ...
    if (map->buckets[posicion] == NULL)
        return;
    free(map->buckets[posicion]->key);  // Libera la memoria asignada al key
    free(map->buckets[posicion]->value);  // Libera la memoria asignada al value
    free(map->buckets[posicion]);  // Libera la memoria asignada al nodo del mapa
